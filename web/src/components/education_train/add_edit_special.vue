<template>
    <el-form ref="form" :model="form" :rules="rules" label-width="130px">
        ...
        <el-form-item label="岗位分类" prop="post_category"> 
            <el-input type="text" v-model="form.post_category"></el-input>
        </el-form-item>
        <el-form-item label="具体工种" prop="work_type"> 
            <el-input type="text" v-model="form.work_type"></el-input>
        </el-form-item> 
        ...
        data(){
            return{
                ...
                rules: {
                    ...
                    post_category: [
                        { required: true, message: '岗位分类', trigger: 'blur' }
                    ], 
                    work_type: [
                        { required: true, message: '具体工种', trigger: 'blur' }
                    ], 
                    qualification_type: [
        ...
        methods: {
            ...
            onSubmit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$request({
                            url: '/specialStaffCertificate/save',
                            method: 'post',
                            data: this.form
                        }).then(res => {
                            this.close();
