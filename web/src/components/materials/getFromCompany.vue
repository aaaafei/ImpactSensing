<template>
    <div>
        <el-form ref="form" :model="form" :rules="rules" label-width="130px"> 
            ...
            <el-row>
                <el-col :span="12">
                    <el-form-item label="领取数量" prop="num"> 
                         <el-input-number v-model="form.num" :min="0" ></el-input-number>
                    </el-form-item>
                </el-col>
                <el-col :span="12"> 
                       <el-form-item label="领取时间" prop="time_in_out">
        ...
        data() {
            return {
                ...
                rules: { 
                    ...
                    time_in_out: [
                        {required: true, message: '领取时间', trigger: 'blur'}
                    ]
                },
                invetory_num: 0
            };
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) { 
                        ...
                        if(!r.test(this.form.num)){
                            this.$message({message: '领取数量至少为1件且为整数！',type: 'warning'});
                            return false;
                        }

                        if(this.invetory_num>0 && this.invetory_num >= this.form.num){
                            this.$request({
                                url: '/materialInventory/getFromCompany',
                                method: 'post',
                                data: this.form
                            }).then(res => { 
