<template>
  ...
  <div id="POST">
   <!-- <el-row>
    ...
    <el-row >
      ...
      <el-col :span="20"  class="right-content">
        <el-col :span="24">
          ...
          <el-button type="danger"  @click="deletePost()" size="small" style="float: right" v-if="checkButtonPerm('position:delete')"> 删除</el-button>
          <el-button type="primary"   size="small"  @click="editPost()" style="float: right" v-if="checkButtonPerm('position:update')">编辑</el-button>
          <el-button type="primary"  @click="addPost()" size="small" style="float: right" v-if="checkButtonPerm('position:add')">新增</el-button>
        </el-col>
        <el-col :span="24">
          <el-table
              v-loading="loading"
              ...
              <el-table-column
                ...
                width="70">
                <template slot-scope="scope">
                  <el-button @click="permAssign(scope.row)" type="text" size="small" v-if="checkButtonPerm('position:authTree')">功能授权</el-button>
                </template>
              </el-table-column>
            </el-table>
          <div class="pageDiv">
    ...
    <addEditPost :addEditVisible= "addEditVisible" :propForm="propForm" :title="title" :isDisabled="isDisabled" :isShowOrg="isShowOrg" :isState="isState" v-on:headCallBack="callBack"></addEditPost>
    <postDetail :detailVisible= "detailVisible" :detailForm="detailForm"  v-on:headCallBackDetail="callBackDetail"></postDetail>
    <permission  :permisData="permisData" :checkedData="checkedData" :permisVisible= "permisVisible" :treePermisLoading="treePermisLoading" v-on:headCallBackPermin="callBackPermin"></permission>
  </div>
</template>
    ...
    methods: {
      ...
      deletePost () { // 删除岗位
        if (this.multipleSelection.length === 1) {  // 当有选择项时
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
            ...
            let theid = this.multipleSelection[0].id; // 岗位id
            this.$request({
              url: '/position/delete/' + theid
            }).then(res => {
              this.$message({
                type: 'success',
      ...
      addPost () { // 新增岗位
        ...
        if (this.treeNode.id === 0) {
          // this.propForm.orgName = '';
          // this.propForm.orgId = '';
          this.$message({
            showClose: true,
            message: '本节点为系统总称，不属于组织，无法添加岗位！',
            type: 'warning'
          });
          return false;
        } else {
          this.propForm.orgName = this.treeNode.label;
          this.propForm.orgId = this.treeNode.id;
        }
        this.propForm.roleId = '';
        this.propForm.roleName = '';
      ...
      editPost () { // 编辑岗位
        if (this.multipleSelection.length === 0) {  // 当无选择项时
          ...
          this.propForm.orgId = pojo.orgId;
          this.propForm.roleId = pojo.roleId;
          this.propForm.roleName = pojo.roleName;
