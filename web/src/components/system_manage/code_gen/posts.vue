<template>
  <div id="POSTS">
    <el-dialog title="新增用户岗位" :visible="postsVisible" @close="close('')">
    <el-row >
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      judgeId (theid) {
        // console.log('判断id是否存在')
        // console.log(theid);
        // console.log(this.thePostsData.postIds);
        // console.log(this.thePostsData.postIds.indexOf(theid));
      ...
      seachPost (data) { // 查询岗位列表
        ...
        let url = this.$api_prefix + '/position/list?pageSize=' + this.page.pageSize + '&pageNum=' + this.page.pageNum;
      ...
      addUserPosts (row) {
        ...
        //   positions = positions + this.multipleSelection[i].id + ',';
        // }
        // positions = positions.substring(0, positions.length-1);
        // options.params.positions = positions;
        this.$http.get(this.$api_prefix + '/user/addPositions', options).then((res) => {
          // 响应成功回调
          if (res.body.code === 401) {
            this.$router.push('/login');
            return false;
