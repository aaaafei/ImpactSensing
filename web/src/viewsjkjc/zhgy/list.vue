  ...
  <div id="USER">    
    <el-row>
      ...
      <el-col :span="24" class="right-content">
        ...
        <el-col :span="24" >
            <div>
                文件类型：<el-select v-model="formInline.bglx" placeholder="请选择" @change="queryData" :clearable="true">
                    <el-option
                        v-for="item in zhgyList"
                        :key="item.code"
                        :label="item.name"
                        :value="item.code">
                    </el-option>
                </el-select>
                <el-button type="danger"  size="small" style="float: right;margin-right:10px" @click="doDelete()" v-if="adminUser">删除</el-button>
                <el-button type="primary"  size="small" style="float: right;margin-right:10px" @click="doAdd()" v-if="adminUser">新增</el-button>
            </div>
        </el-col> 
            ...
            <el-table ref="multipleTable" :data="tableData3" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="bglx" label="类型" width="150">
                  <template slot-scope="scope" v-if="scope.row.bglx">
                      {{scope.row.bglx=='1'?'养护知识库文件列表':(scope.row.bglx=='2'?'管养建议列表':'')}}
                  </template>
              </el-table-column>
              ...
              <el-table-column label="操作" fixed="right" width="250">
                <template slot-scope="scope" class="theoprations">
                    <el-button size="mini" type="success" @click="doEdit(scope.row)">修改</el-button>
    ...
    <el-dialog
      title="智慧管养文件管理"
      :visible.sync="zhgyDialog.visible"
      width="30%"
      :before-close="zhgyDialogClose">
        <el-form :inline="true" :model="zhgyForm" :rules="rules4cus" ref="zhgyForm" label-width="100px" size="mini">
            <el-form-item label="文件类型" prop="bglx">
                <el-select v-model="zhgyForm.bglx" placeholder="请选择" style="width:200px">
                    <el-option
                        v-for="item in zhgyList"
                        :key="item.code"
                        :label="item.name"
            ...
            <el-form-item label="文件上传" prop="full_path" v-if="!zhgyForm.uuid">
                <el-upload ref="attachment_path" :http-request="req" :before-remove="beforeRemove" :on-preview="handlePreview" :on-exceed="handleExceed" :limit="1" :file-list="fileList" :show-file-list="true" action="" v-show="!zhgyForm.name">                
                    <el-button size="mini" type="primary" plain>点击上传</el-button>
                </el-upload>
                <label v-show="zhgyForm.name">{{zhgyForm.name}}</label>
            </el-form-item>
        </el-form>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        zhgyList:[{code:'1',name:'养护知识库文件列表'},{code:'2',name:'管养建议列表'}],//报告类型
        tableData3: [],
        selectUUIDs:[],
        page: { // 分页的数据信息
            pageNum: 1,
    ...
    methods: {
        ...
        queryData(){
            var url = '/zhgy/getPageData/' + this.page.pageNum + '/' + this.page.pageSize;
                this.$request({
                url: url,
                method: 'post',
                data:{
        ...
        doDelete(row){
            if(this.selectUUIDs.length==0){
                ...
                this.$confirm('确认删除所选数据吗？','提示',{
                    ...
                    this.$request({
                        url: '/zhgy/delete/' + this.selectUUIDs.join(',') +"/",
                        method:'get'
                    }).then(res => {
                        this.$message({
                            type: 'success',
        ...
        zhgyDialogClose(){
            this.zhgyDialog.visible=false;
            this.queryData();
        },
        previewFile(filePath){//页面查看附件内容
        ...
        updateSave(){//表单提交
            this.$refs['zhgyForm'].validate((valid) => {
                let fd = new FormData();
                fd.append('bglx',this.zhgyForm.bglx);
                fd.append('full_path',this.file); 
                fd.append('report_date',this.zhgyForm.report_date); 
                if (valid) {
                    var url = "/zhgy/add/";
                    if(this.zhgyForm.uuid){
                        url = "/zhgy/update/";
                        fd.append('uuid',this.zhgyForm.uuid);
